...
/*        sumMoney:function(){
        ...
        excel:function(){
            if (vm.tids.length==0){
                vm.tids=["all",vm.q.status];
            }
            window.location.href=baseURL + "sys/systradetrade/excel?tids="+vm.tids;
            vm.tids = [];
        },
        ...
        getCommission:function(tid){
            $("#commissionList").modal();
            $.get(baseURL + "sys/systradetrade/getCommission/"+tid, function(r){
                vm.commissionList = r.list;
            });
        },
	}
});